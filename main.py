...
def parse_args_and_config():
    ...
    parser.add_argument('--runner', type=str, default='CovarianceRunner', help='The runner to execute')
    parser.add_argument('--config', type=str, default='mnist_cov.yml', help='Path to the config file')
    ...
    parser.add_argument('--noise', type=float, default=0.5, help="denoising score matching noise level")
    parser.add_argument('--rank', type=int, default=50, help='low rank model') #10

    args = parser.parse_args()
    if args.runner == "CovarianceRunner":
        args.doc = args.doc + "_covariance"

    if args.rank > 0:
        args.doc = args.doc + "_rank{}".format(args.rank)
    ...
    if not args.test:
        if not args.resume_training and not (args.runner == 'PixelCNNPPSamplerRunner') and not (
                args.runner == 'PixelCNNPP_ELBO_Runner'):
...
def main():
    ...
    config.gaussian_sigma = args.noise # pass noise level to save a copy
